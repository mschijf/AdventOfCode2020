...
class PuzzleSolver(test: Boolean) : PuzzleSolverAbstract(test) {
    ...
    override fun resultPartTwo(): String {
        var blackSet = initialBlackSet()//input.inputLines.map { DirectionSeries(it) }.map { it.finalTile() }.toSet()
...
//            println("Day ${it+1}: ${blackSet.count()}")
    ...
    private fun initialBlackSet(): Set<Hexagon> {
        val directionSeriesList = input.inputLines.map { DirectionSeries(it) }
